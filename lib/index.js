...
async function initConfig() {
  ...
  // 询问是否对所有平台使用相同的用户名
  const useSameUsername = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'same',
      message: '是否对所有平台使用相同的用户名?',
      default: false
    }
  ]);
      ...
      const answer = await inquirer.prompt([
        {
          type: 'input',
          name: 'username',
          message: `请输入 ${platformInfo.icon} ${platformInfo.name} 的用户名:`,
          default: existingInfo?.username || repoInfo.gitUserName,
          validate: input => input.trim().length > 0 || '用户名不能为空'
        }
      ]);
  ...
  if (saveConfig(config)) {
    ...
    Object.entries(config.platforms).forEach(([platform, config]) => {
      const platformInfo = PLATFORMS[platform];
      console.log(`  ${platformInfo.icon} ${platform}: ${config.username}`);
    });
  }
